...
workflow {
    ...
    def plot_input_ch = info_ch.info.combine(genomic_ch)
...
process PLOT_VARIANTS {
    publishDir("${params.outdir}/${id}"), mode: 'copy'
    input:
        tuple val(id), path("${id}_domain.tsv"), path("${id}_transcript.tsv"),
        path(genomic_file), path(genomic_noncoding_file), path(exonic_file),
        path(sv_file), path(encode_file), path(refseq_file),
        path(gnomAD_file), path(UK_Biobank_file), path(colour_phenotypes_file)
    output:
